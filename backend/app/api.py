...
class WorkflowStore:
    ...
    def add_template(self, template: WorkflowTemplate) -> TemplateRecord:
        versions = [record.template.version for record in self.templates.values() if record.template.key == template.key]
        if versions:
            template = template.model_copy(update={"version": max(versions) + 1})
    ...
    def create_new_version(self, template_id: UUID, template: WorkflowTemplate) -> TemplateRecord:
        current = self.get_template(template_id)
        next_version = max(
            record.template.version
            for record in self.templates.values()
            if record.template.key == current.template.key
        ) + 1
        versioned = template.model_copy(update={"key": current.template.key, "version": next_version})
        record = TemplateRecord(id=uuid4(), template=versioned, created_at=datetime.now(timezone.utc))
        self.templates[record.id] = record
        return record
...
@router.put("/templates/{template_id}", response_model=TemplateRecord, status_code=status.HTTP_201_CREATED)
def update_template(template_id: UUID, template: WorkflowTemplate, request: Request) -> TemplateRecord:
    return store.create_new_version(template_id, template)
...
def list_pending_approvals(request: Request) -> list[WorkflowInstance]:
    ...
    for record in store.instances.values():
        ...
        if role == Role.ADMIN or any(approval.approver == actor or approval.approver_role == actor for approval in current_stage):
...
ROLE_RULES: dict[str, set[Role]] = {
    ...
    "PUT /api/templates/{template_id}": {Role.ADMIN},
